...
/*!
 * LoRaMacPacket is used to communication with upper layers
 */
uint8_t LoRaMacDevEui[] = LORAWAN_DEVICE_EUI;
uint8_t LoRaMacRecvBuffer[LORAWAN_APP_DATA_MAX_SIZE];
uint8_t LoRaMacRecvLength;
uint8_t *const LoRaMacPacketBufferPtr = &AppData[0];
uint8_t *const LoRaMacPacketLengthPtr = &AppDataSize;
...
{
    if( LoRaMacAppDataFrameSent == true )
    {
        LoRaMacPacketEmpty = true;
        LoRaMacPacketAckReceived = false;
    }
...
static void McpsIndication( McpsIndication_t *mcpsIndication )
{
    ...
    if( mcpsIndication->RxData == true )
    {
        ...
        case 2:
            if( mcpsIndication->BufferSize > 0 && mcpsIndication->BufferSize <= LORAWAN_APP_DATA_MAX_SIZE )
            {
                memcpy(LoRaMacRecvBuffer, mcpsIndication->Buffer, mcpsIndication->BufferSize);
                LoRaMacRecvLength = mcpsIndication->BufferSize;
                LoRaMacRecvPending = true;
            }
            break;
        case 224:
...
void LoRaMacProcessLoopOnes( void )
{
