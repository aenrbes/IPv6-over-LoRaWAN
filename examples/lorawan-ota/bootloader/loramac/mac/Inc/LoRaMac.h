...
typedef enum eDeviceClass
{
     ...
     */
    CLASS_A,//上行后，紧跟两个短暂的下行接收窗口，服务器在其他任何时间下行传输的数据，都得等终端下一次上行时再发，那样终端才能够收的到 -- 最省电
    /*!
     * LoRaWAN device class B
     *
     * LoRaWAN Specification V1.0.1, chapter 8ff
     */
    CLASS_B,//在class A的随机接收窗口的基础上，还会在指定时间内打开一个接收窗口，node需要从网关接收时间同步信号指标beacon，使服务器可以知道终端正在监听
    /*!
     * LoRaWAN device class C
     *
     * LoRaWAN Specification V1.0.1, chapter 17ff
     */
    CLASS_C,//一直打开接收窗口，只在发送的短暂瞬间关闭接收窗口 -- 最耗电，但下行数据延时最短
}DeviceClass_t;
