...
update_firmware( uint8_t ota_slot )
{
  ...
  for (uint8_t sector_num=0; sector_num<15; sector_num++) {
...
{
  uint32_t jump_addr = destination_address + OTA_RESET_VECTOR ;
  ...
  __asm("LDR R0, [%[dest]]"::[dest]"r"(jump_addr)); //  Load the destination address
  __asm("ORR R0, #1");                                        //  Make sure the Thumb State bit is set.
  __set_MSP(*(__IO uint32_t*)destination_address);
